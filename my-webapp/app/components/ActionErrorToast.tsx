...
export function ActionErrorToast({
  ...
  return (
    <div className="fixed bottom-4 left-1/2 z-60 w-auto max-w-[calc(100%-2rem)] -translate-x-1/2 animate-in fade-in slide-in-from-bottom-4 duration-300">
      <div className="rounded-2xl border-2 border-[#8B3A3A] bg-[#FFF6F4] px-4 py-3 shadow-xl">
        <div className="flex items-center gap-3">
          <div className="rounded-full bg-[#FFEBEE] p-2 shrink-0">
            <AlertCircle className="w-5 h-5 text-[#8B3A3A]" />
          </div>

          <div className="min-w-0 flex-1">
            <p className="text-sm font-bold text-[#6B2020] whitespace-nowrap">{title}</p>
            {description && <p className="mt-1 text-sm text-[#5C4A2F]">{description}</p>}
